...
# ==========================================
# GEMM (128, 768) * (768, 128)
total_mac_ops = 128 * 768 * 128  # 12,582,912 MACs
...
bars1 = ax1.bar(labels, [bytes_per_mac_valu, bytes_per_mac_cim], color=['#e74c3c', '#2ecc71'], width=0.5, edgecolor='black', linewidth=1.2)

for bar in bars1:
    yval = bar.get_height()
    ax1.text(bar.get_x() + bar.get_width()/2, yval + 0.5, f'{yval:.2f} Bytes/MAC', ha='center', va='bottom', fontsize=12, fontweight='bold')

ax1.annotate('7.5x Less Memory Traffic!', 
            xy=(1, bytes_per_mac_cim), xytext=(1, bytes_per_mac_cim + 6),
...
rects1 = ax1.bar(x - width/2, [throughput_valu, throughput_cim], width, label='Effective MACs / Cycle', color='#3498db', edgecolor='black', linewidth=1.2)
ax1.set_ylabel('Absolute Throughput (MACs / Cycle)', fontsize=12, fontweight='bold', color='#2980b9')
ax1.tick_params(axis='y', labelcolor='#2980b9')
ax1.set_ylim(0, 14)

# Utilization (Right Y-axis)
ax2 = ax1.twinx()
rects2 = ax2.bar(x + width/2, [util_valu, util_cim], width, label='Hardware MAC Utilization (%)', color='#f39c12', edgecolor='black', linewidth=1.2)
ax2.set_ylabel('Hardware Utilization (%)', fontsize=12, fontweight='bold', color='#d35400')
ax2.tick_params(axis='y', labelcolor='#d35400')
ax2.set_ylim(0, 20)

ax1.set_xticks(x)
ax1.set_xticklabels(labels, fontsize=12, fontweight='bold')
ax1.set_title('Throughput vs. Hardware Utilization', fontsize=14, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('fig2_throughput_utilization.png', dpi=300)
plt.close(fig2)